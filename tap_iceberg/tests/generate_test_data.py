...
if __name__ == "__main__":
    ...
    try:
        catalog.load_table(identifier=SORTED_TABLE_IDENTIFIER)
        catalog.load_table(identifier=PARTITIONED_TABLE_IDENTIFIER)
        catalog.drop_table(identifier=SORTED_TABLE_IDENTIFIER)
        catalog.drop_table(identifier=PARTITIONED_TABLE_IDENTIFIER)
    except Exception:
        pass
    ...
    sorted_table = catalog.create_table(
        identifier=SORTED_TABLE_IDENTIFIER, schema=iceberg_schema, sort_order=sort_order
    )
    ...
    partitioned_table = catalog.create_table(
        identifier=PARTITIONED_TABLE_IDENTIFIER,
    ...
    partitioned_table.overwrite(pyarrow_table)
    # Sort the rows based on the sort order defined for the sorted Iceberg table.
    pyarrow_table = pyarrow_table.sort_by("updated_at")
    sorted_table.overwrite(pyarrow_table)
    # Assert row count
    assert len(sorted_table.scan().to_arrow()) == 10_000
    assert len(partitioned_table.scan().to_arrow()) == 10_000
